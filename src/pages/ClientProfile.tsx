...
type AppointmentEvent = Tables<"calendar_events"> & {
  provider?: {
...
const ClientProfile = () => {
  ...
  // Appointments
  const [pendingAppointments, setPendingAppointments] = useState<AppointmentEvent[]>([]);
  const [pastAppointments, setPastAppointments] = useState<AppointmentEvent[]>([]);
  ...
  const loadAppointments = async (uid: string) => {
    ...
    const [{ data: pending }, { data: past }] = await Promise.all([pendingQuery, pastQuery]);

    setPendingAppointments((pending as AppointmentEvent[]) || []);
    setPastAppointments((past as AppointmentEvent[]) || []);
  };
  ...
  const AppointmentCard = ({ apt, isPast = false }: { apt: AppointmentEvent; isPast?: boolean }) => (
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {apt.appointment_type || apt.title || "Session"}
                </p>
              </div>
              <Badge
                variant={
