...
import { ManualVerificationRequestFlow } from "../components/ManualVerificationRequestFlow";
import { supabase } from "@/integrations/supabase/client";

export default function FaceScanProviderWithManualFallback() {
  ...
  const [vaiPhotoUrl, setVaiPhotoUrl] = useState<string>("");
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      // Try automatic face verification
      // TODO: Call actual face verification API here
      // For now, simulate failure to show manual fallback
      const verificationSuccess = false; // Replace with actual verification result

      if (!verificationSuccess) {
        // Determine failure reason (in real implementation, get from verification API response)
        const failureReasonFromAPI: 'system_failure' | 'individual_issue' | 'failed_verification' = 'failed_verification';
        setFailureReason(failureReasonFromAPI);

        // Create session first (needed for manual verification)
      ...
      // If verification succeeds, proceed normally
      const { data: session, error: sessionError } = await supabase
        .from('vai_check_sessions')
        .insert({
          provider_id: user.id,
      ...
      // Check if error indicates manual verification option
      if (error.verification_failed && error.can_use_manual_verification) {
        setFailureReason(error.failure_reason || 'failed_verification');
        setShowManualFallback(true);
        setIsVerifying(false);
      } else {
        toast({
  ...
  if (showManualFallback && capturedImage && vaiPhotoUrl && sessionId) {
    return (
