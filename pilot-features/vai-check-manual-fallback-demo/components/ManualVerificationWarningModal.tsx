...
interface ManualVerificationWarningModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
const failureReasonDescriptions = {
  system_failure: "A technical glitch prevented automatic verification from working properly. This is not a security concern - your identity is still valid.",
  individual_issue: "The user was unable to complete automatic verification. This may be due to camera issues, lighting conditions, or temporary technical problems.",
  failed_verification: "The automatic face verification did not pass. The photos may not match, or there may be other verification issues."
};

export const ManualVerificationWarningModal = ({ 
  open, 
  onClose, 
  onConfirm, 
  failureReason, 
  role,
  loading = false 
}: ManualVerificationWarningModalProps) => {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            Important information about manual verification and liability.
          </DialogDescription>
        </DialogHeader>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                {isOwner ? 'Request Manual Verification' : 'Proceed to Review'}
              </>
            )}
          </Button>
        </DialogFooter>
